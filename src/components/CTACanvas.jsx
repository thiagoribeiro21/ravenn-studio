...
function NebulaField() {
  ...
  const [basePos, colors] = useMemo(() => {
    ...
    const coreCount = Math.floor(PARTICLE_COUNT * 0.7);
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      if (i < coreCount) {
        ...
        const r = 2.8 + Math.random() ** 0.4 * 2.7;
        const theta = Math.random() * Math.PI * 2;
        const phi = Math.acos(2 * Math.random() - 1);
        x = r * Math.sin(phi) * Math.cos(theta);
      ...
      else if (t < 0.65) tmp.lerpColors(cMid, cOuter, (t - 0.25) / 0.4);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: geometria criada uma única vez (buffers pré-alocados em refs, mutados in-place no loop de frame)
  const geo = useMemo(() => {
