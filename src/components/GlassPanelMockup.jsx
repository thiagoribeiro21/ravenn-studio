...
export default function GlassPanelMockup({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: `cycleKey` é o gatilho de reinício sincronizado (não é lido no corpo, só dispara o efeito quando o grupo reinicia)
  useEffect(() => {
    const video = videoRef.current;
  ...
  useEffect(() => {
    ...
    const report = () => {
      if (video.duration) onDuration(video.duration);
    };
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) setSiteStarted(true);
      },
      { threshold: 0.15 },
    );
    io.observe(el);
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: parallax puramente decorativo por mouse, sem consequência funcional
    <div
      ref={containerRef}
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          inset: '-14%',
          background:
            'radial-gradient(ellipse 70% 60% at 50% 55%, rgba(124,58,237,0.30) 0%, transparent 70%)',
          filter: 'blur(44px)',
          pointerEvents: 'none',
          zIndex: 0,
        }}
      ...
      <div
        ...
        style={{
          ...
          padding: 1,
          background:
            'linear-gradient(160deg, rgba(255,255,255,0.18), rgba(124,58,237,0.14) 45%, rgba(255,255,255,0.03))',
          boxShadow: ['0 44px 110px -26px rgba(0,0,0,0.78)', '0 0 0 1px rgba(0,0,0,0.35)'].join(
            ', ',
          ),
        }}
      >
