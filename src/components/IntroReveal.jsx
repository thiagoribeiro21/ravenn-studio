...
import { markIntroDone, SESSION_KEY } from '../introStore';
...
export default function IntroReveal() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: roda só uma vez na montagem; `finish` é recriada por render mas o timer sempre chama a versão atual via closure
  useEffect(() => {
  ...
  return (
    <AnimatePresence>
      {visible && (
        // `role="button"` + `tabIndex`/`onKeyDown`: era só `onClick` num
        // `<div>` — funcionava no mouse/touch, mas ficava inalcançável por
        // teclado (sem Tab, sem Enter/Space pra pular), falha real de
        // WCAG 2.1.1. A cortina cobre a tela inteira antes do resto do
        // site existir no DOM, então quem navega só por teclado ficaria
        // travado aqui até o timer automático (`SPLIT_DUR`) resolver
        // sozinho — mudo, mas não mais um beco sem saída.
        // biome-ignore lint/a11y/useSemanticElements: <button> traria chrome/estilo padrão do UA num overlay full-bleed; div+role="button"+onKeyDown já é acessível (ver nota acima)
        <div
          onClick={skip}
          role="button"
          tabIndex={0}
        ...
        >
          ...
          <motion.div
            ...
            style={{
              ...
              background:
                'linear-gradient(90deg, transparent, #A78BFA, #C4B5FD, #A78BFA, transparent)',
          ...
          >
            ...
            <div
              style={{
                ...
                borderRadius: '50%',
                background:
                  'radial-gradient(circle, rgba(124,58,237,0.35) 0%, rgba(76,29,149,0.14) 45%, transparent 72%)',
                filter: 'blur(60px)',
              }}
            />
          </motion.div>
          ...
          >
            <div
              style={{ position: 'relative', width: 'clamp(140px,16vw,220px)', aspectRatio: '1' }}
            >
              ...
              <motion.div
                ...
                style={{
                  ...
                  borderRadius: '50%',
                  background:
                    'radial-gradient(circle, rgba(233,213,255,0.9) 0%, rgba(167,139,250,0.55) 32%, transparent 70%)',
                  filter: 'blur(4px)',
                  pointerEvents: 'none',
                }}
              />
